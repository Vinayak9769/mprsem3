    ...
    const fetchBallStats = async () => {
      try {
        const response = await fetch(
          'https://jameagle.pythonanywhere.com/stats/match/1359536', {
            headers: {
  ...
  useEffect(() => {
    const fetchMatchDetails = async () => {
      try {
        ...
        const data = await response.json();
        const match = data.find((match) => match.match_id === 1359536); // Get the specific match data
